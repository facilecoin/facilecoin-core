...
uint256& uint256::SetCompact_dummy(uint32_t nCompact, bool* pfNegative, bool* pfOverflow){
	uint32_t nWord = nCompact;
	*this = nWord;
	if (pfNegative)
		*pfNegative = false;
	if (pfOverflow)
...
uint32_t uint256::GetCompact_dummy(bool fNegative) const
{
	uint32_t nCompact = 0;
	uint256 bn = *this;
