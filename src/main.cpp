...
#if defined(NDEBUG)
# error "FacileCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "FacileCoin Signed Message:\n";
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    // Check the header
    if (
		//block.GetHash() != Params().HashGenesisBlock() && 
		!CheckProofOfWork(block.GetHash(), block.nBits, block.sol))
...
void ThreadScriptCheck() {
    RenameThread("facilecoin-scriptch");
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s (time=%d) progress=%f  cache=%u\n",
	...
	//LogPrintf("UPDATE_DEBUG %s\n", chainActive.Tip()->sol.ToString);  
