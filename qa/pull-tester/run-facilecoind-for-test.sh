...
#
DATADIR="/opt/clean_build/facilecoin-0.10.2/.facilecoin"
rm -rf "$DATADIR"
...
"/opt/clean_build/facilecoin-0.10.2/src/facilecoind" -connect=0.0.0.0 -datadir="$DATADIR" -rpcuser=user -rpcpassword=pass -listen -keypool=3 -debug -debug=net -logtimestamps -checkmempool=0 -relaypriority=0 -port=$PORT -whitelist=127.0.0.1 -regtest -rpcport=`expr $PORT + 1` &
BITCOIND=$!
